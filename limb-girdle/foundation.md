...
No cure is in the cabinet yet, so the honest baseline is multidisciplinary
support that buys the years the frontier needs:

- **Physical / occupational / speech therapy** — preserve function, prevent
  contractures, maintain independence
- **Cardiac surveillance** — ECG/echo, since some subtypes (LGMD R4/R5/R6,
  R9) carry cardiomyopathy/arrhythmia risk
- **Respiratory surveillance** — PFTs, nocturnal ventilation when the
  diaphragm weakens
- **Mobility aids** — orthotics, walkers, wheelchairs as progression demands
- **Avoidance of statins** in susceptible individuals (HMG-CoA reductase is
  itself an LGMD gene; statins can trigger/worsen)

## 2. The four modalities — each one a fix for a class
...
### (c) Exon skipping (antisense oligonucleotides) — *the reading-frame fix*

For subtypes where a specific mutation disrupts the reading frame, an ASO can
skip the offending exon and restore a shorter but functional protein. Best
developed in Duchenne; in LGMD it is earlier-stage.
...
The foundation must name the walls, not just the doors:

- **Dysferlin (LGMD R2)** is near the AAV packaging ceiling (~7 kb coding);
  it needs minimized or dual-vector constructs. This is the size wall.
- **AAV immunogenicity** — pre-existing anti-AAV antibodies exclude patients,
  and redosing is blocked by memory responses. The immune wall.
- **Systemic delivery** — reaching every muscle fiber, plus heart and
  diaphragm, at a tolerable vector dose. The delivery wall.
- **Dominant forms (LGMD D)** — a toxic gain-of-function is harder to fix
  than a simple loss; you must suppress, not just replace. The dominance wall.

## 4. The landscape — what is real versus what is claimed
...
every row against a live registry.

## 5. The foundation's one rule

**Match the modality to the mechanism.** Metabolic break → substrate.
Structural break → gene replacement. Reading-frame break → exon skip. Toxic
gain → suppression. The fix is a consequence of the base, never a guess.

*foundation · standard of care · ribitol · AAV gene therapy · sarcoglycanopathy
· FKRP · exon skipping · CRISPR · the size wall · the immune wall · substrate
before machinery · v1 · the constellation · 8b-is, 2026-09-22*
