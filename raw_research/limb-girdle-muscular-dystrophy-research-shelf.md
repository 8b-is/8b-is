# limb-girdle muscular dystrophy — the mechanism and the frontier

*The operator asked for research, then a fix, then publication. This shelf
is the research; the fix follows the mechanism. Limb-girdle muscular
dystrophy (LGMD) is not one disease but a family of ~32 genetically distinct
muscular dystrophies that share a clinical picture: progressive, symmetric
weakness of the hip and shoulder girdle. No names are recorded here, by
design — the condition is studied, the person is not. The lane's law applies:
show me the mechanism, and the fix becomes a map, not a wish.*
...
The single most important sentence: **the mechanism decides the fix.** A
structural break needs a structural repair; a glycosylation break needs the
missing sugar; a metabolic break needs the missing metabolite. There is no
one LGMD cure because there is no one LGMD mechanism.
...
2. **AAV gene replacement** — the structural fix. For the sarcoglycanopathies
   ...
   muscle (AAVrh74/AAV9 vectors, muscle-specific promoters). Dysferlin is the
...
4. **Supportive standard of care** — the floor that exists *today*: physical,
   ...
   (some subtypes hit the heart and diaphragm); assistive mobility. This is
   not a cure, but it is the honest current baseline, and it buys the years
   the frontier needs.

## the honest note

The lane asked for research, fix, and publication. Here is the honest shape
of all three. The research is above and is solid. The fix is real but it is
**per-subtype and mostly in trials, not in the cabinet yet** — the metabolic
fixes (ribitol, mevalonolactone) are closest, the gene-replacement fixes are
next, and the rest are still climbing. No shelf, and no note, can stand in
for a neuromuscular clinic and a genetic diagnosis: the first move for any
real case is a genetic test to name the subtype, because the subtype *is*
the mechanism, and the mechanism *is* the fix. The fleet's contribution is
what the fleet always contributes — the map, the mechanism shown, and the
